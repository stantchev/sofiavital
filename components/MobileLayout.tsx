...
// ── Profile Pill Scroller ─────────────────────────────────────────────────────
function ProfileScroller({
  profile, onChange,
}: { profile: string; onChange: (p: string) => void }) {
  return (
    <div style={{
        ...
        <div style={{ padding: "0 18px 14px", borderBottom: "1px solid var(--border-subtle)", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary)", fontFamily: "var(--font-display)", letterSpacing: "-0.02em", lineHeight: 1 }}>
                {district.name}
              </div>
        ...
        <div style={{ overflowY: "auto", padding: "14px 18px 24px", flex: 1 }}>
            ...
            return (
              <div key={key} style={{ marginBottom: 12 }}>
                ...
                <div style={{ height: 4, background: "var(--bg-raised)", borderRadius: 2, overflow: "hidden" }}>
                  <div style={{ width: `${val}%`, height: "100%", background: `linear-gradient(90deg, ${col}88, ${col})`, borderRadius: 2, transition: "width 0.6s ease" }} />
                </div>
              </div>
            );
          })}
          ...
          <div style={{ marginTop: 16, display: "flex", flexWrap: "wrap", gap: 6 }}>
            {indicators.filter(([k]) => (district[k] as number) >= 75).map(([k, label]) => (
              <span key={k} style={{ background: "rgba(6,95,70,0.3)", color: "#6ee7b7", border: "1px solid rgba(52,211,153,0.2)", fontSize: 10, padding: "3px 9px", borderRadius: "var(--radius-full)" }}>✓ {label}</span>
            ))}
            {indicators.filter(([k]) => (district[k] as number) < 55).map(([k, label]) => (
              <span key={k} style={{ background: "rgba(120,53,15,0.3)", color: "#fb923c", border: "1px solid rgba(251,146,60,0.2)", fontSize: 10, padding: "3px 9px", borderRadius: "var(--radius-full)" }}>⚠ {label}</span>
            ))}
          </div>
        </div>
      </div>
...
// ── Ranking Tab ───────────────────────────────────────────────────────────────
function RankingTab({
  weights, profile, onSelectDistrict,
        ...
        {LAYERS.map((layer) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 12, fontWeight: active ? 600 : 400, color: active ? "var(--text-primary)" : "var(--text-secondary)" }}>
                  {layer.label}
                </div>
              ...
              {active && <div style={{ width: 6, height: 6, borderRadius: "50%", background: layer.color, boxShadow: `0 0 6px ${layer.color}` }} />}
            </button>
          );
        })}
      </div>
...
function ChatTab() {
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      setError("Няма връзка."); setMessages(messages);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {error && (
          <div style={{ padding: "8px 12px", borderRadius: "var(--radius-md)", background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", fontSize: 12, color: "#f87171", textAlign: "center" }}>{error}</div>
        )}
...
function CustomWeightsSheet({
  weights, onChange, onClose,
            ...
            return (
              <div key={key}>
                ...
                <input
                  ...
                  style={{ width: "100%", accentColor: "#34d399", height: 4 }}
                />
              </div>
            );
          })}
...
export default function MobileLayout() {
  ...
  const handleLayerToggle = useCallback((id: string) => {
    ...
    setActiveTab("map"); // jump back to map to see the layer
  }, []);
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ position: "absolute", inset: 0 }}>
            ...
            {!selectedDistrict && (
              <div style={{
                ...
                fontSize: 11, color: "var(--text-muted)", pointerEvents: "none", zIndex: 10,
                whiteSpace: "nowrap",
              }}>Докосни район за детайли</div>
            )}
            {/* Legend */}
            <div style={{
              position: "absolute", bottom: 70, left: 12,
              background: "var(--bg-surface)", border: "1px solid var(--border-subtle)",
              borderRadius: "var(--radius-md)", padding: "8px 12px", zIndex: 10,
            }}>
        ...
        {/* Ranking tab */}
        {activeTab === "ranking" && (
          <RankingTab weights={weights} profile={profile} onSelectDistrict={handleSelectDistrict} />
        )}

        {/* Layers tab */}
        {activeTab === "layers" && (
          <LayersTab activeId={activeLayer} onToggle={handleLayerToggle} />
        )}
      ...
      {showCustomWeights && (
        <CustomWeightsSheet
          weights={customW}
          onChange={handleCustomWeight}
          onClose={() => setShowCustomW(false)}
        />
      )}
